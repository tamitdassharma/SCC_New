CLASS LHC_/ESRCC/I_ALLOCATIONKEY_S DEFINITION INHERITING FROM CL_ABAP_BEHAVIOR_HANDLER.
  PRIVATE SECTION.
    METHODS:
      AUGMENT FOR MODIFY
        IMPORTING
          ENTITIES_CREATE FOR CREATE AllocationKeyAll\_AllocationKey
          ENTITIES_UPDATE FOR UPDATE AllocationKey.
ENDCLASS.

CLASS LHC_/ESRCC/I_ALLOCATIONKEY_S IMPLEMENTATION.
  METHOD AUGMENT.
    DATA: text_for_new_entity      TYPE TABLE FOR CREATE /ESRCC/I_AllocationKey\_AllocationKeyText,
          text_for_existing_entity TYPE TABLE FOR CREATE /ESRCC/I_AllocationKey\_AllocationKeyText,
          text_update              TYPE TABLE FOR UPDATE /ESRCC/I_AllocationKeyText.
    DATA: text_tky_link  TYPE STRUCTURE FOR READ LINK /ESRCC/I_AllocationKey\_AllocationKeyText,
    ...
    LOOP AT entity-%TARGET ASSIGNING FIELD-SYMBOL(<target>).
      INSERT VALUE #( %CID_REF = <target>-%CID
                      %IS_DRAFT = <target>-%IS_DRAFT
                        %KEY-Allocationkey = <target>-%KEY-Allocationkey
                      %TARGET = VALUE #( (
    ...
    ENDLOOP.
    MODIFY AUGMENTING ENTITIES OF /ESRCC/I_AllocationKey_S
      ENTITY AllocationKey
        CREATE BY \_AllocationKeyText
        FROM text_for_new_entity.

    IF entities_update IS NOT INITIAL.
      READ ENTITIES OF /ESRCC/I_AllocationKey_S
