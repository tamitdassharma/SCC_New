CLASS LHC_/ESRCC/I_SRVTG_S DEFINITION INHERITING FROM CL_ABAP_BEHAVIOR_HANDLER.
  PRIVATE SECTION.
    METHODS:
      AUGMENT FOR MODIFY
        IMPORTING
          ENTITIES_CREATE FOR CREATE TransactionGrpAll\_TransactionGrp
          ENTITIES_UPDATE FOR UPDATE TransactionGrp.
ENDCLASS.

CLASS LHC_/ESRCC/I_SRVTG_S IMPLEMENTATION.
  METHOD AUGMENT.
    DATA: text_for_new_entity      TYPE TABLE FOR CREATE /ESRCC/I_SrvTg\_TransactionGrpText,
          text_for_existing_entity TYPE TABLE FOR CREATE /ESRCC/I_SrvTg\_TransactionGrpText,
          text_update              TYPE TABLE FOR UPDATE /ESRCC/I_SrvTgText.
    DATA: text_tky_link  TYPE STRUCTURE FOR READ LINK /ESRCC/I_SrvTg\_TransactionGrpText,
    ...
    LOOP AT entity-%TARGET ASSIGNING FIELD-SYMBOL(<target>).
      INSERT VALUE #( %CID_REF = <target>-%CID
                      %IS_DRAFT = <target>-%IS_DRAFT
                        %KEY-Transactiongroup = <target>-%KEY-Transactiongroup
                      %TARGET = VALUE #( (
    ...
    ENDLOOP.
    MODIFY AUGMENTING ENTITIES OF /ESRCC/I_SrvTg_S
      ENTITY TransactionGrp
        CREATE BY \_TransactionGrpText
        FROM text_for_new_entity.

    IF entities_update IS NOT INITIAL.
      READ ENTITIES OF /ESRCC/I_SrvTg_S
