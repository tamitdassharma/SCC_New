CLASS LHC_/ESRCC/I_SRTYPE_S DEFINITION INHERITING FROM CL_ABAP_BEHAVIOR_HANDLER.
  PRIVATE SECTION.
    METHODS:
      AUGMENT FOR MODIFY
        IMPORTING
          ENTITIES_CREATE FOR CREATE ServiceTypeAll\_ServiceType
          ENTITIES_UPDATE FOR UPDATE ServiceType.
ENDCLASS.

CLASS LHC_/ESRCC/I_SRTYPE_S IMPLEMENTATION.
  METHOD AUGMENT.
    DATA: text_for_new_entity      TYPE TABLE FOR CREATE /ESRCC/I_SrType\_ServiceTypeText,
          text_for_existing_entity TYPE TABLE FOR CREATE /ESRCC/I_SrType\_ServiceTypeText,
          text_update              TYPE TABLE FOR UPDATE /ESRCC/I_SrTypeText.
    DATA: text_tky_link  TYPE STRUCTURE FOR READ LINK /ESRCC/I_SrType\_ServiceTypeText,
    ...
    LOOP AT entity-%TARGET ASSIGNING FIELD-SYMBOL(<target>).
      INSERT VALUE #( %CID_REF = <target>-%CID
                      %IS_DRAFT = <target>-%IS_DRAFT
                        %KEY-Srvtype = <target>-%KEY-Srvtype
                      %TARGET = VALUE #( (
    ...
    ENDLOOP.
    MODIFY AUGMENTING ENTITIES OF /ESRCC/I_SrType_S
      ENTITY ServiceType
        CREATE BY \_ServiceTypeText
        FROM text_for_new_entity.

    IF entities_update IS NOT INITIAL.
      READ ENTITIES OF /ESRCC/I_SrType_S
