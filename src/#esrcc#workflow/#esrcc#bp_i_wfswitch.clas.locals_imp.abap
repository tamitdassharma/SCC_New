...
CLASS LHC_RAP_TDAT_CTS IMPLEMENTATION.
  METHOD GET.
    result = mbc_cp_api=>rap_table_cts( table_entity_relations = VALUE #(
                                         ( entity = 'WorkflowSwitch' table = '/ESRCC/WF_SWITCH' )
                                       ) ).
  ENDMETHOD.
ENDCLASS.
...
CLASS LHC_/ESRCC/I_WFSWITCH_S IMPLEMENTATION.
  ...
  METHOD GET_GLOBAL_AUTHORIZATIONS.
    AUTHORITY-CHECK OBJECT 'S_TABU_NAM' ID 'TABLE' FIELD '/ESRCC/I_WFSWITCH' ID 'ACTVT' FIELD '02'.
  ...
  PRIVATE SECTION.
    METHODS:
      VALIDATERECORDCHANGES FOR VALIDATE ON SAVE
        IMPORTING
          KEYS FOR WorkflowSwitch~ValidateRecordChanges,
