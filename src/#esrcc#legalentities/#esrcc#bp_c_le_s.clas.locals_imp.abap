CLASS LHC_/ESRCC/I_LE_S DEFINITION INHERITING FROM CL_ABAP_BEHAVIOR_HANDLER.
  PRIVATE SECTION.
    METHODS:
      AUGMENT FOR MODIFY
        IMPORTING
          ENTITIES_CREATE FOR CREATE LegalEntityAll\_LegalEntity
          ENTITIES_UPDATE FOR UPDATE LegalEntity.
ENDCLASS.

CLASS LHC_/ESRCC/I_LE_S IMPLEMENTATION.
  METHOD AUGMENT.
    DATA: text_for_new_entity      TYPE TABLE FOR CREATE /ESRCC/I_LE\_LegalEntityText,
          text_for_existing_entity TYPE TABLE FOR CREATE /ESRCC/I_LE\_LegalEntityText,
          text_update              TYPE TABLE FOR UPDATE /ESRCC/I_LEText.
    DATA: text_tky_link  TYPE STRUCTURE FOR READ LINK /ESRCC/I_LE\_LegalEntityText,
    ...
    LOOP AT entity-%TARGET ASSIGNING FIELD-SYMBOL(<target>).
      INSERT VALUE #( %CID_REF = <target>-%CID
                      %IS_DRAFT = <target>-%IS_DRAFT
                        %KEY-Legalentity = <target>-%KEY-Legalentity
                      %TARGET = VALUE #( (
    ...
    ENDLOOP.
    MODIFY AUGMENTING ENTITIES OF /ESRCC/I_LE_S
      ENTITY LegalEntity
        CREATE BY \_LegalEntityText
        FROM text_for_new_entity.

    IF entities_update IS NOT INITIAL.
      READ ENTITIES OF /ESRCC/I_LE_S
