CLASS LHC_/ESRCC/I_BUSDIV_S DEFINITION INHERITING FROM CL_ABAP_BEHAVIOR_HANDLER.
  PRIVATE SECTION.
    METHODS:
      AUGMENT FOR MODIFY
        IMPORTING
          ENTITIES_CREATE FOR CREATE BusinessDivisionAll\_BusinessDivision
          ENTITIES_UPDATE FOR UPDATE BusinessDivision.
ENDCLASS.

CLASS LHC_/ESRCC/I_BUSDIV_S IMPLEMENTATION.
  METHOD AUGMENT.
    DATA: text_for_new_entity      TYPE TABLE FOR CREATE /ESRCC/I_BusDiv\_BusinessDivisioText,
          text_for_existing_entity TYPE TABLE FOR CREATE /ESRCC/I_BusDiv\_BusinessDivisioText,
          text_update              TYPE TABLE FOR UPDATE /ESRCC/I_BusDivText.
    DATA: text_tky_link  TYPE STRUCTURE FOR READ LINK /ESRCC/I_BusDiv\_BusinessDivisioText,
    ...
    ENDLOOP.
    MODIFY AUGMENTING ENTITIES OF /ESRCC/I_BusDiv_S
      ENTITY BusinessDivision
        CREATE BY \_BusinessDivisioText
        FROM text_for_new_entity.

    IF entities_update IS NOT INITIAL.
      READ ENTITIES OF /ESRCC/I_BusDiv_S
