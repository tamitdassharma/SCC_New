CLASS LHC_/ESRCC/I_FUNCTIONALAREAS_S DEFINITION INHERITING FROM CL_ABAP_BEHAVIOR_HANDLER.
  PRIVATE SECTION.
    METHODS:
      AUGMENT FOR MODIFY
        IMPORTING
          ENTITIES_CREATE FOR CREATE FunctionalAreasAll\_FunctionalAreas
          ENTITIES_UPDATE FOR UPDATE FunctionalAreas.
ENDCLASS.

CLASS LHC_/ESRCC/I_FUNCTIONALAREAS_S IMPLEMENTATION.
  METHOD AUGMENT.
    DATA: text_for_new_entity      TYPE TABLE FOR CREATE /ESRCC/I_FunctionalAreas\_FunctionalAreasText,
          text_for_existing_entity TYPE TABLE FOR CREATE /ESRCC/I_FunctionalAreas\_FunctionalAreasText,
          text_update              TYPE TABLE FOR UPDATE /ESRCC/I_FunctionalAreasText.
    DATA: text_tky_link  TYPE STRUCTURE FOR READ LINK /ESRCC/I_FunctionalAreas\_FunctionalAreasText,
    ...
    LOOP AT entity-%TARGET ASSIGNING FIELD-SYMBOL(<target>).
      INSERT VALUE #( %CID_REF = <target>-%CID
                      %IS_DRAFT = <target>-%IS_DRAFT
                        %KEY-FunctionalArea = <target>-%KEY-FunctionalArea
                      %TARGET = VALUE #( (
    ...
    ENDLOOP.
    MODIFY AUGMENTING ENTITIES OF /ESRCC/I_FunctionalAreas_S
      ENTITY FunctionalAreas
        CREATE BY \_FunctionalAreasText
        FROM text_for_new_entity.

    IF entities_update IS NOT INITIAL.
      READ ENTITIES OF /ESRCC/I_FunctionalAreas_S
