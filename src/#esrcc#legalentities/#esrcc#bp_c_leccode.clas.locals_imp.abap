CLASS LHC_/ESRCC/I_LECCODE_S DEFINITION INHERITING FROM CL_ABAP_BEHAVIOR_HANDLER.
  PRIVATE SECTION.
    METHODS:
      AUGMENT FOR MODIFY
        IMPORTING
          ENTITIES_CREATE FOR CREATE LeToCompanyCodeAll\_LeToCompanyCode
          ENTITIES_UPDATE FOR UPDATE LeToCompanyCode.
ENDCLASS.

CLASS LHC_/ESRCC/I_LECCODE_S IMPLEMENTATION.
  METHOD AUGMENT.
    DATA: text_for_new_entity      TYPE TABLE FOR CREATE /ESRCC/I_LeCcode\_CompanyCodeText,
          text_for_existing_entity TYPE TABLE FOR CREATE /ESRCC/I_LeCcode\_CompanyCodeText,
          text_update              TYPE TABLE FOR UPDATE /ESRCC/I_CcodeText.
    DATA: text_tky_link  TYPE STRUCTURE FOR READ LINK /ESRCC/I_LeCcode\_CompanyCodeText,
    ...
    ENDLOOP.
    MODIFY AUGMENTING ENTITIES OF /ESRCC/I_LeCcode_S
      ENTITY LeToCompanyCode
        CREATE BY \_CompanyCodeText
        FROM text_for_new_entity.

    IF entities_update IS NOT INITIAL.
      READ ENTITIES OF /ESRCC/I_LeCcode_S
