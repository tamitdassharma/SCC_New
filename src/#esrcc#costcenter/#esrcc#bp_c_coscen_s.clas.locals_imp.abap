CLASS LHC_/ESRCC/I_COSCEN_S DEFINITION INHERITING FROM CL_ABAP_BEHAVIOR_HANDLER.
  PRIVATE SECTION.
    METHODS:
      AUGMENT FOR MODIFY
        IMPORTING
          ENTITIES_CREATE FOR CREATE CostCenterAll\_CostCenter
          ENTITIES_UPDATE FOR UPDATE CostCenter.
ENDCLASS.

CLASS LHC_/ESRCC/I_COSCEN_S IMPLEMENTATION.
  METHOD AUGMENT.
    DATA: text_for_new_entity      TYPE TABLE FOR CREATE /ESRCC/I_CosCen\_CostCenterText,
          text_for_existing_entity TYPE TABLE FOR CREATE /ESRCC/I_CosCen\_CostCenterText,
          text_update              TYPE TABLE FOR UPDATE /ESRCC/I_CosCenText.
    DATA: text_tky_link  TYPE STRUCTURE FOR READ LINK /ESRCC/I_CosCen\_CostCenterText,
    ...
    LOOP AT entity-%TARGET ASSIGNING FIELD-SYMBOL(<target>).
      INSERT VALUE #( %CID_REF = <target>-%CID
                      %IS_DRAFT = <target>-%IS_DRAFT
                        ...
                        %KEY-Costcenter = <target>-%KEY-Costcenter
    ...
    ENDLOOP.
    MODIFY AUGMENTING ENTITIES OF /ESRCC/I_CosCen_S
      ENTITY CostCenter
        CREATE BY \_CostCenterText
        FROM text_for_new_entity.

    IF entities_update IS NOT INITIAL.
      READ ENTITIES OF /ESRCC/I_CosCen_S
        ENTITY CostCenter BY \_CostCenterText
