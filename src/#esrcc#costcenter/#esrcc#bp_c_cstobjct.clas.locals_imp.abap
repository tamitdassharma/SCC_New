CLASS LHC_/ESRCC/I_CSTOBJCT_S DEFINITION INHERITING FROM CL_ABAP_BEHAVIOR_HANDLER.
  PRIVATE SECTION.
    METHODS:
      AUGMENT FOR MODIFY
        IMPORTING
          ENTITIES_CREATE FOR CREATE CostObjectAll\_CostObject
          ENTITIES_UPDATE FOR UPDATE CostObject.
ENDCLASS.

CLASS LHC_/ESRCC/I_CSTOBJCT_S IMPLEMENTATION.
  METHOD AUGMENT.
    DATA: text_for_new_entity      TYPE TABLE FOR CREATE /ESRCC/I_CstObjct\_CostObjectText,
          text_for_existing_entity TYPE TABLE FOR CREATE /ESRCC/I_CstObjct\_CostObjectText,
          text_update              TYPE TABLE FOR UPDATE /ESRCC/I_CstObjctText.
    DATA: text_tky_link  TYPE STRUCTURE FOR READ LINK /ESRCC/I_CstObjct\_CostObjectText,
    ...
    LOOP AT entity-%TARGET ASSIGNING FIELD-SYMBOL(<target>).
      INSERT VALUE #( %CID_REF = <target>-%CID
                      %IS_DRAFT = <target>-%IS_DRAFT
                        %KEY-CostObjectUuid = <target>-%KEY-CostObjectUuid
                      %TARGET = VALUE #( (
    ...
    ENDLOOP.
    MODIFY AUGMENTING ENTITIES OF /ESRCC/I_CstObjct_S
      ENTITY CostObject
        CREATE BY \_CostObjectText
        FROM text_for_new_entity.

    IF entities_update IS NOT INITIAL.
      READ ENTITIES OF /ESRCC/I_CstObjct_S
