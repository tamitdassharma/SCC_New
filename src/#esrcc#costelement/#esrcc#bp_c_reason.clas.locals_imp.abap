CLASS LHC_/ESRCC/I_REASON_S DEFINITION INHERITING FROM CL_ABAP_BEHAVIOR_HANDLER.
  PRIVATE SECTION.
    METHODS:
      AUGMENT FOR MODIFY
        IMPORTING
          ENTITIES_CREATE FOR CREATE ReasonAll\_Reason
          ENTITIES_UPDATE FOR UPDATE Reason.
ENDCLASS.

CLASS LHC_/ESRCC/I_REASON_S IMPLEMENTATION.
  METHOD AUGMENT.
    DATA: text_for_new_entity      TYPE TABLE FOR CREATE /ESRCC/I_Reason\_ReasonText,
          text_for_existing_entity TYPE TABLE FOR CREATE /ESRCC/I_Reason\_ReasonText,
          text_update              TYPE TABLE FOR UPDATE /ESRCC/I_ReasonText.
    DATA: text_tky_link  TYPE STRUCTURE FOR READ LINK /ESRCC/I_Reason\_ReasonText,
    ...
    LOOP AT entity-%TARGET ASSIGNING FIELD-SYMBOL(<target>).
      INSERT VALUE #( %CID_REF = <target>-%CID
                      %IS_DRAFT = <target>-%IS_DRAFT
                        %KEY-Reasonid = <target>-%KEY-Reasonid
                      %TARGET = VALUE #( (
    ...
    ENDLOOP.
    MODIFY AUGMENTING ENTITIES OF /ESRCC/I_Reason_S
      ENTITY Reason
        CREATE BY \_ReasonText
        FROM text_for_new_entity.

    IF entities_update IS NOT INITIAL.
      READ ENTITIES OF /ESRCC/I_Reason_S
